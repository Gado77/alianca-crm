...
export async function extractFichaAction(_prev: FichaImportState, formData: FormData): Promise<FichaImportState> {
  ...
  if (!apiKey) {
    return fichaErrorState("Configure OPENAI_API_KEY no ambiente do servidor para ler fichas por imagem.");
  }
  ...
  try {
            ...
            content: [
              {
                type: "input_text",
                text: [
                  "Extraia os dados desta ficha da Aliança Motos para cadastro de lead.",
                  "A ficha pode ter dois modelos visuais diferentes.",
                  "Leia apenas o que estiver escrito ou marcado. Se houver duvida, deixe vazio.",
                  "Converta CPF e telefone para somente digitos quando possivel.",
                  "Converta data de nascimento para YYYY-MM-DD quando legivel.",
                  "Mapeie CNH: sim/categoria A -> a, categoria A e B -> ab, nao possui/nao marcado -> nao_possui.",
                  "Modelos possiveis incluem AZ1, AZ125-ALFA e AZ 160 X-TREME. Preserve o nome visivel.",
                  "Pagamento: financiamento, cartao, a_vista, consorcio ou outro.",
                  "Coloque informacoes nao encaixadas em notes, inclusive situacao da simulacao, data da ficha, condicoes de entrada e consultor.",
                ].join("\n"),
              },
          ...
          format: {
            ...
            schema: {
              ...
              properties: {
                ...
                intended_down_payment: { type: ["number", "string"] },
    ...
    if (!response.ok) {
      console.error("extractFichaAction.openai", response.status, await response.text());
      return fichaErrorState("Nao foi possivel ler a ficha agora. Tente novamente.");
    }

    const payload = await response.json();
    const outputText = extractResponseText(payload);
    if (!outputText) return fichaErrorState("A leitura nao retornou dados. Tente outra foto.");

    const raw = JSON.parse(outputText) as Record<string, unknown>;
    return {
      ok: true,
      message: "Ficha lida. Confira os campos antes de salvar.",
      extracted: normalizeFichaLead(raw),
    };
  } catch (error) {

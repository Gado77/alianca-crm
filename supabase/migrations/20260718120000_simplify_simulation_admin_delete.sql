...
begin
  ...
  perform public.add_timeline_event(
    ...
    'Simulação criada',
    'Simulação registrada com resultado ' || new.result::text || '.',
    jsonb_build_object(
  ...
  if new.result = 'negado' and lead_assigned_user_id is not null then
    ...
    elsif denial_text like '%score%' or denial_text like '%baixo%' or denial_text like '%baixa%' or denial_text like '%por fora%' then
...
drop policy if exists "simulations_admin_delete_only" on public.simulations;
create policy "simulations_admin_delete_only"
